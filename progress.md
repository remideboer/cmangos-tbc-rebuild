...
| 11 EventAI | p0_junit | Gherkin Garrick **103** spell **7164** (`TP-SL11-001`); `FactorySelector.selectAI` (ScriptName / pet / AIName / Guard / permit EventAI / Null); creature melee after `BASEATTACKTIME` in range (no chase); EVENT_KILL. In-memory `load(scripts)` only. **Not `p0_client`.** |
