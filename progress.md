...
| 12 Combat math | p0_junit | `MeleeTableTest` 5% miss/dodge/parry/block vs same-level NPC; yellow-NPC miss; miss vs player `× 0.04`; NPC dodge 0.1 / parry 0.6 skill; NPC block no +skill when defense ahead; player avoid from `PLAYER_*_PERCENTAGE` (no +5); player vs NPC>10 glance 10+(defense−skill) and low/high-end damage; sitting player remaining table is crit; player/creature crit 2×; NPC crush 150% when skill deficit ≥ 15; `hitInfo` CRITICALHIT / CRUSHING |
