...
public final class WorldSession {
    ...
    public void handle(World world, int opcode, byte[] payload) {
        ...
        switch (opcode) {
            ...
            case Opcodes.CMSG_SET_FACTION_ATWAR -> handleSetFactionAtWar(in);
            ...
            case Opcodes.CMSG_CHANNEL_OWNER -> ChannelHandler.owner(this, in);
    ...
    private void handleShowingHelm() {
        togglePlayerFlag(Player.PLAYER_FLAGS_HIDE_HELM);
    }
    ...
    /** CharacterHandler::HandleSetFactionAtWarOpcode — ReputationMgr::SetAtWar, no SMSG. */
    private void handleSetFactionAtWar(WowBuffer in) {
        if (in.remaining() < 5) {
            return;
        }
        int listId = in.getU32();
        int flag = in.getU8();
        player.reputations.setAtWar(listId, flag != 0);
    }
    ...
    private void togglePlayerFlag(int mask) {
        int flags = player.getInt(UpdateFields.PLAYER_FLAGS) ^ mask;
        player.setInt(UpdateFields.PLAYER_FLAGS, flags);
        var upd = UpdateBuilder.maybeCompress(UpdateBuilder.values(player, UpdateFields.PLAYER_FLAGS));
        send(upd.opcode(), upd.payload());
    }
    ...
    private void handleQuestgiverStatusMultiple(World world) {
        world.content.questGiverStatusMultiple(player, world.map(player.mapId, player.instanceId), this::send);
    }
    ...
    private void handleQuestLogRemove(World world, WowBuffer in) {
        world.content.removeQuest(player, in, this::send);
    }
    ...
    private void handleGoUse(World world, WowBuffer in) {
        ...
        if (guid == PvpObjectives.GO_ZM_CENTER_N) {
            if (world.outdoorPvp.claimZmGraveyard(player, true)) {
                flushWorldStates(world.outdoorPvp.drainWorldStates());
            }
        }
